    ...
    if (event.key === "user") {
      if (event.newValue) {
        ...
        if (this.state.isLoggedIn) {
          ...
          this.clearSlideInterval();
          if (this.state.publicImages.length > 0) {
            this.slideInterval = setInterval(this.nextSlide.bind(this), 5000);
          }
        }
      }
  ...
  render() {
    ...
    const blogData = [
      ...
      },
      {
        ...
        fullDescription: "Professional mobile app development services to create stunning applications for iOS and Android platforms. Mobile app development requires careful consideration of platform-specific guidelines, performance optimization, and user experience. Whether you're building native apps or using cross-platform frameworks like React Native or Flutter, understanding the fundamentals of mobile development is crucial. Key considerations include app store optimization, push notifications, offline functionality, and security best practices. The mobile landscape is constantly evolving, so staying updated with the latest trends and technologies is essential for success.",
        author: "VOAT Network",
            ...
            <div className="landing-page-container">
              ...
              <div className="landing-page-blog-pagination">
                <div className="landing-page-pagination-info">
                  Showing {((this.state.currentBlogPage - 1) * this.state.blogsPerPage) + 1} to {Math.min(this.state.currentBlogPage * this.state.blogsPerPage, blogData.length)} of {blogData.length} posts
                </div>
                <div className="landing-page-pagination-controls">
                  ...
                  {Array.from({ length: Math.ceil(blogData.length / this.state.blogsPerPage) }, (_, i) => i + 1).map(pageNum => (
                    <button
                      key={pageNum}
                  ...
                  <button
                    className={`landing-page-pagination-btn ${this.state.currentBlogPage === Math.ceil(blogData.length / this.state.blogsPerPage) ? 'disabled' : ''}`}
                    onClick={() => this.handleBlogPageChange(this.state.currentBlogPage + 1)}
                    disabled={this.state.currentBlogPage === Math.ceil(blogData.length / this.state.blogsPerPage)}
                  >
                    Next
                  </button>
                </div>
